...
export default function QuestionBankDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ── Render: Config panel (Create Quiz modal) ───────────────────────────────
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="text-sm text-muted-foreground">
                from <span className="text-white font-bold">{maxCount}</span> available questions
              </p>
            </div>
  ...
  // ── Render: Main bank detail ───────────────────────────────────────────────
  return (
    <div className="space-y-6">
      {confirmModal}
      {/* Header */}
      <div className="flex items-center gap-4">
        <button onClick={() => router.push("/teacher/question-bank")} className="p-2 hover:bg-secondary rounded-lg transition text-muted-foreground hover:text-white">
          <ArrowLeft size={20} />
        </button>
